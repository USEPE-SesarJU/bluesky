...
""" The version has deep learning algorithm."""
""" Main Change
    - Deep Learning algorithm implemented for conflict prediction.
    - NN command is added.
"""
""" Some bugs are fixed in version 4.
    Summary boxes are updated in general and local score plots to prevent misunderstanding of number of conflicts.
"""
...
# Define MLP --------------------------------------------------------------------
# Parameters for NN & dir
nn_dir = os.path.join(os.path.dirname(__file__), '@NN')
trained_NN_params = {
    'N_HIDDEN': 256,
    'N_LAYERS': 3,
    'TRAINED_BATCH_SIZE': 64,
    'TRAINED_EPOCH_NUM': 30
    }

# The construction of trained NN
class myMLP(nn.Module):
    def __init__(self, input, n_layers, hidden=64, output=2, device='cpu'):
        super().__init__()
...
class USEPE_ML(core.Entity):
    ...
    def debug(self):
        ...
        print("\t NN is {}".format(self.use_nn))
        debug_log.log([['USEPE_ML NN', self.use_nn]])
    ...
    def conf_logger(self):
        ...
        # Conflict counter
        self.countconf = [confpair for confpair in self.prevconf if confpair not in self.countconf]
        ...
        # Conflict counting -----------------------
        for unique_pair in self.countconf:
            a, b = unique_pair
            a, b = sorted([a, b])
            # register "conf count"
            if '{}&{}'.format(a, b) not in self.conf_counter.keys():
    ...
    def output_wpt(self, scenario_path_list):
        ...
        # Force reset
        bs.sim.reset()
    # WPT Scenario Generator
    def generate_rnd_traffic(self, num_aircrafts, num_direction, distance):
        base_lat, base_lon, base_alt = 0, 0, 800
        # Generate lattice
        lattice_wpt = []
        wpt2pos = {} # wpt_name: [lat, lon, alt]
        print('Generate lattice path')
        for i in range(num_direction):
            for j in range(num_direction):
                lat = base_lat + i*distance
                lon = base_lon + j*distance
                alt = base_alt
                wpt_name = 'WPT{}_{}'.format(i, j)

                lattice_wpt.append([lat, lon, alt])
                wpt2pos[wpt_name] = [lat, lon, alt]

                print('\t(col-{} row-{}):{}'.format(i, j, lattice_wpt[-1]))
        # Def get neighbors
        wpt2neighbor = {} #wpt_name: [wpt1, wpt2, ...]
        for wpt_name in wpt2pos.keys():
            wpt2neighbor[wpt_name] = list()
            base_pos = wpt2pos[wpt_name]
            # Up
            up_pos = [base_pos[0], base_pos[1]-distance, base_pos[2]]
            # Down
            down_pos = [base_pos[0], base_pos[1]+distance, base_pos[2]]
            # Left
            left_pos = [base_pos[0]-distance, base_pos[1], base_pos[2]]
            # Right
            right_pos = [base_pos[0]+distance, base_pos[1], base_pos[2]]

            for neighbor_pos in [up_pos, down_pos, left_pos, right_pos]:
                neighbor_wpt = [k for k,v in wpt2pos.items() if v == neighbor_pos]
                if len(neighbor_wpt) != 0:
                    wpt2neighbor[wpt_name].append(neighbor_wpt[0])

        last_lat, last_lon = lattice_wpt[-1][0], lattice_wpt[-1][1]
        # Generate traffic scenario file
        output_scn_fname = 'USEPE_ML{}TRAFFIC_Generated_sim.scn'.format(num_aircrafts)
        output_f = open(os.path.join('./scenario', output_scn_fname), 'w')
        # Add init info
        output_f.write('00:00:00.00> AREA {} {} {} {}\n'.format(base_lat, base_lon, last_lat, last_lon))
        output_f.write('00:00:00.00> PAN {} {}\n'.format(base_lat, base_lon))
        output_f.write('00:00:00.00> ZOOM 5\n')
        output_f.write('00:00:00.00> FF\n')
        output_f.write('\n')
        # Append traffic wpt for each aircraft
        is_set_defwpts = []
        start_sec = 0
        offset_sec = 0
        str_end_time = '01:00:00.00'
        for i in range(num_aircrafts):
            # Write cre
            if i != 0:
                start_sec += offset_sec
            # Calc hours
            str_hour = math.floor(start_sec / 3600)
            str_min = math.floor((start_sec - str_hour*3600) / 60)
            str_sec = math.floor(start_sec - (str_hour*3600 + str_min*60))
            str_msec = (start_sec - (str_hour*3600 + str_min*60 + str_sec))

            # Output to str
            str_hour = "%02.f"%str_hour
            str_min = "%02.f"%str_min
            str_sec = "%02.f"%str_sec
            str_msec = "%02.f"%str_msec
            str_time = '{}:{}:{}.{}'.format(str_hour, str_min, str_sec, str_msec)

            # Init sim aircraft name
            aircraft_name = 'SIM{:05}'.format(i)

            # Init start wpt & goal wpt
            start_wpt_name = random.choice(list(wpt2pos.keys()))
            #start_wpt_idx = [idx for idx, wpt_name in enumerate(wpt2pos.keys()) if wpt_name == start_wpt_name][0]
            start_wpt = wpt2pos[start_wpt_name]
            # Defining random WPTs
            crr_wpt_name = start_wpt_name
            add_wpt_name_list = []
            for j in range(num_direction):
                neighbors = wpt2neighbor[crr_wpt_name]
                next_wpt_name = random.choice(list(neighbors))
                if next_wpt_name not in is_set_defwpts:
                    next_wpt = wpt2pos[next_wpt_name]
                    output_f.write('{}>DEFWPT {} {} {}\n'.format(str_time, next_wpt_name, next_wpt[0], next_wpt[1]))
                    is_set_defwpts.append(next_wpt_name)

                add_wpt_name_list.append(next_wpt_name)
                crr_wpt_name = next_wpt_name

            output_f.write('\n')
            # Cre aircrafts
            output_f.write('{}>CRE {} {} {} {} 500 4\n'.format(str_time, aircraft_name, start_wpt[0], start_wpt[1], start_wpt[2]))
            output_f.write('\n')
            # Add wpt
            for wpt_name in add_wpt_name_list:
                output_f.write('{}>ADDWPT {} {}\n'.format(str_time, aircraft_name, wpt_name))
            output_f.write('\n')

            print('{} Start pos:{} path:{}'.format(aircraft_name, start_wpt_name, add_wpt_name_list))
            # Del aircraft at the goal wpt
            output_f.write('{0}>{1} AT {2} DO DEL {1}\n'.format(str_time, aircraft_name, add_wpt_name_list[-1]))

            output_f.write('\n')

        output_f.write('{}>RESET'.format(str_end_time))
        output_f.close()
        print('Output scenario file: {}'.format(os.path.join('./scenario', output_scn_fname)))
    ...
    def usepe_ml(self, cmd="Help", cmd2=True):
        ...
        # --------------------------------------------------------------------------------------------
        # Generate scenario file of traffic command --------------------------------------------------
        elif cmd == 'TRAFFIC':
            inputs = cmd2.split('/')
            num_aircrafts = int(inputs[0])
            num_directions = int(inputs[1])
            distance = float(inputs[2])

            self.generate_rnd_traffic(num_aircrafts, num_directions, distance)
        # --------------------------------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------------------------------
        # ON/OFF use NN command ----------------------------------------------------------------------
        elif cmd == 'NN':
            if allloggers[loggers[0]].isopen() is False:
                self.is_basic = True
                for x, log_name in enumerate(loggers):
                    stack.stack(f'{loggers[x]} BASIC')
                    allloggers[log_name].start()
            # ON
            if self.use_nn is False:
                self.use_nn = True
                return True, f'USEPE_ML with NN is enabled.'
            # OFF
            else:
                self.use_nn = False
                return True, f'USEPE_ML with NN is disabled.'
        # --------------------------------------------------------------------------------------------
        elif cmd == 'OFF':
            ...
            if self.is_basic is True and self.is_extra is False and self.is_extend is False:
                if allloggers[loggers[0]].isopen():
                    ...
                    # + NN ==================
                    if self.use_nn is True:
                        output_csv_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        self.use_nn = False
            ...
            if self.is_basic is True and self.is_extra is True and self.is_extend is False:
                ...
                if allloggers[loggers[0]].isopen():
                    ...
                    if self.use_nn is True:
                        output_csv_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        output_csv_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=True)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=True)
                        self.use_nn = False
            ...
            if self.is_basic is True and self.is_extra is False and self.is_extend is True:
                ...
                if allloggers[loggers[0]].isopen():
                    ...
                    if self.use_nn is True:
                        output_csv_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        output_csv_with_pred_conf_from_NN(log_fname, mode='los', conf_list_in_sec=self.los_num_in_sec, use_entire_content=False)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='los', conf_list_in_sec=self.los_num_in_sec, use_entire_content=False)
                        self.use_nn = False
            ...
            if self.is_basic is True and self.is_extra is True and self.is_extend is True:
                ...
                if allloggers[loggers[0]].isopen():
                    ...
                    if self.use_nn is True:
                        ...
                        output_csv_with_pred_conf_from_NN(log_fname, mode='los', conf_list_in_sec=self.los_num_in_sec, use_entire_content=False)
                        output_csv_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=True)
                        output_csv_with_pred_conf_from_NN(log_fname, mode='los', conf_list_in_sec=self.los_num_in_sec, use_entire_content=True)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=False)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='los', conf_list_in_sec=self.los_num_in_sec, use_entire_content=False)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='conf', conf_list_in_sec=self.conf_num_in_sec, use_entire_content=True)
                        output_plot_with_pred_conf_from_NN(log_fname, mode='los', conf_list_in_sec=self.los_num_in_sec, use_entire_content=True)
                        self.use_nn = False
...
# -----------------------------------------------------
# [For NN] Generate CSV: pred conflict using trained NN
def pairwise_aircraft_pred_NN_to_csv(file_name, pair_aircraft_dic, pair_aircraft_sec_dic, mode, sep_category_name, conf_list_in_sec):
    if mode == 'conf':
        mode_name = 'Conflict'
    elif mode == 'los':
        mode_name = 'LoS'
    
    if sep_category_name == 'Spatial Features':
        nn_input = 3 # alt, lon, lat
    else:
        nn_input = 15
     # Define column of csv
    csv_column = ['Index', 'Time[s]', 'Aircraft1', 'Aircraft2',
                  'NOT {}'.format(mode_name), '{}'.format(mode_name), 'Actually:{}'.format(mode_name), 'NN Correct']
    csv_log = []
    # Init CUDA ------------------------------------------------------------------
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    # Set NN path & Get model ----------------------------------------------------
    nn_path = os.path.join(nn_dir, 'myMLP[N_Hidden={0} N_Layers={1}] with config(BATCH_SIZE={2} CONTENT={3})_epoch={4}.pth'.format(
        trained_NN_params['N_HIDDEN'], trained_NN_params['N_LAYERS'], trained_NN_params['TRAINED_BATCH_SIZE'],
        sep_category_name, trained_NN_params['TRAINED_EPOCH_NUM']
    ))
    predMLP = myMLP(input=nn_input, n_layers=trained_NN_params['N_LAYERS'], hidden=trained_NN_params['N_HIDDEN'], device=device).to(device)
    predMLP.load_state_dict(torch.load(nn_path))
    predMLP.eval()
    # Load pairwise dataset
    for idx_i, pairname in enumerate(pair_aircraft_dic.keys()):
        # Get each aircraft info
        aircraft_a, aircraft_b = pairname.split('&')
        aircraft_a_data = pair_aircraft_dic[pairname][0] # shape = (sec_length, FEATURES)
        aircraft_b_data = pair_aircraft_dic[pairname][1]

        input_x = torch.FloatTensor(aircraft_a_data).to(device)
        input_y = torch.FloatTensor(aircraft_b_data).to(device)

        with torch.no_grad():
            preds = predMLP(input_x, input_y)
        preds = F.softmax(preds, dim=1)
        preds = preds.data.cpu()

        for idx_j, p in enumerate(preds):
            sec = float(pair_aircraft_sec_dic[pairname][idx_j])
            # Get prediction
            had_conf = p[1].item()
            not_had_conf = p[0].item()
            # Get answer
            conf_pair_list = conf_list_in_sec[sec]
            answer_had_conf = True if pairname in conf_pair_list else False
            # See correct
            if (answer_had_conf is True and had_conf > not_had_conf) or (answer_had_conf is False and not_had_conf > had_conf):
                is_NN_correct = 'pred: CORRECT'
            else:
                is_NN_correct = 'pred: NOT CORRECT'

            csv_log.append([idx_i+1, sec, aircraft_a, aircraft_b, not_had_conf, had_conf, answer_had_conf, is_NN_correct])
...
def general_spatial_conf_csv(log_fname, conf_count_fname):
    ...
    for i, id in enumerate(general.keys()):
        if id not in conf.keys():
            conf[id] = 0
            conf_norm[id] = 0
...
# Generate CSV Func ---------------------------------------------------------------
# Exports NN phase between pair aircraft
def output_csv_with_pred_conf_from_NN(log_fname, conf_list_in_sec, mode='conf', use_entire_content=True):
    # About file path -------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------
    # ----------------------------------------------------------------------------
    pair_aircraft_dic = {} # pairname: [[a] [b]]
    pair_aircraft_sec_dic = {} # pairname: sec_list
    for idx, pair in enumerate(itertools.combinations(unique_ids, 2)):
        aircraft_a, aircraft_b = pair
        # print('Extract pair {}&{} in {}'.format(aircraft_a, aircraft_b, log_name))
        # print('\t for output CSV with sep={} mode={}'.format(sep_category_name, mode))
        key = '{}&{}'.format(aircraft_a, aircraft_b)
        # Init pairwise sec dic
        pair_aircraft_sec_dic[key] = []
        ...
        for j, (index_a, index_b) in enumerate(zip(a_mask, b_mask)): # adjust mask len (for broadcast)
            ...
            for k, content in enumerate(separation_contents):
                ...
                aircraft_b_data[j][k] = data_b[content]
            # get sec
            pair_aircraft_sec_dic[key].append(sec)
        # Set pairwise info dic
        pair_aircraft_dic[key] = [aircraft_a_data.copy(), aircraft_b_data.copy()]
        # -----------------------------------------------------------------------------------------
    # Set output plot dir
    output_csv_dir = os.path.dirname(log_fname)

    # Output csv
    csv_fname = os.path.join(output_csv_dir, 'USEPE_ML with predict NN {} {} for {}'.format(sep_category_name, count_category_name, log_name))
    csv_fname = data_export(csv_fname)
    # file_name, pair_aircraft_dic, pair_aircraft_sec_dic, mode, sep_category_name
    pairwise_aircraft_pred_NN_to_csv(file_name=csv_fname, pair_aircraft_dic=pair_aircraft_dic, pair_aircraft_sec_dic=pair_aircraft_sec_dic,
                                     conf_list_in_sec=conf_list_in_sec,
                                     mode=mode, sep_category_name=sep_category_name)
...
def local_spatial_conf_plot(log_fname, conf_count_fname):
   ...
   # norm_count_dic = get_norm_MinMax_sep_score(count_dic.copy())
    ...
    tot_conf_single = sum(list(conf.values())) / 2
    ave_conf_single = sum(list(conf.values())) / (2 * aircraft)
    ave_norm_conf_single = sum(list(conf_norm.values())) / (2 * aircraft)
    # Set figure
    fig, ax = plt.subplots(figsize=(10, 8), dpi=1000)
    ...
    for idx, aircraft_name in enumerate(local.keys()):
        conf = conf_norm[aircraft_name] if aircraft_name in conf_norm.keys() else 0
        sep = local_norm[aircraft_name] if aircraft_name in local_norm.keys() else 0
        Y0.append(conf)
        Y1.append(sep)
    Y0 = np.array(Y0)
    Y1 = np.array(Y1)
...
def local_spatial_los_plot(log_fname, los_count_fname):
   ...
   # norm_count_dic = get_norm_MinMax_sep_score(count_dic.copy())
    ...
    avg_sim = sum(list(local.values())) / aircraft
    norm_avg_sim = sum(list(local_norm.values())) / aircraft
    tot_los_single = sum(list(los.values())) / 2
    ave_los_single = sum(list(los.values())) / (2*aircraft)
    ave_norm_los_single = sum(list(los_norm.values())) / (2*aircraft)
    # Set figure
    fig, ax = plt.subplots(figsize=(10, 8), dpi=1000)
    X = np.array(np.arange(1, len(local)+1))
    ax.xaxis.get_major_locator().set_params(integer=True) # Makes X axis ticks integer values, Aircrafts
    ...
    Y0, Y1 = [], []
    for idx, aircraft_name in enumerate(local.keys()):
        los = los_norm[aircraft_name] if aircraft_name in los_norm.keys() else 0
        sep = local_norm[aircraft_name] if aircraft_name in local_norm.keys() else 0
        Y0.append(los)
        Y1.append(sep)
    Y0 = np.array(Y0)
...
def general_spatial_conf_plot(log_fname, conf_count_fname):
   ...
   # norm_count_dic = get_norm_MinMax_sep_score(count_dic.copy())
    ...
    for idx, aircraft_name in enumerate(general.keys()):
        conf = conf_norm[aircraft_name] if aircraft_name in conf_norm.keys() else 0
        sep = general_norm[aircraft_name] if aircraft_name in general_norm.keys() else 0
        Y0.append(conf)
        Y1.append(sep)
    Y0 = np.array(Y0)
    Y1 = np.array(Y1)
...
# [For NN] Generate pred conflict plot
def pairwise_aircraft_pred_NN_to_plot(file_name, pair_aircraft_dic, pair_aircraft_sec_dic, mode, sep_category_name, conf_list_in_sec):
    if mode == 'conf':
        mode_name = 'Conflict'
    elif mode == 'los':
        mode_name = 'LoS'
    ...
    aircrafts_list = []
    # Init Common Dicts
    pairname2pred_answer = {} # pairname: [pred_in_sec1, pred_in_sec_2, ....]
    pairname2pred = {} # pairname: [pred_in_sec1, pred_in_sec_2, ....]
    pairname2target = {} # pairname: [pred_in_sec1, pred_in_sec_2, ....]
    # Set figure
    # 1st Plot (Pairs -vs- Times) -------------------------------------------------------------
    print("Simulation Analysis using MLP, Plot (Pairs - vs - Times), is started.")
    pair_time_plot_fname = '{}[Time -vs- Pairs]'.format(file_name)
    fig, ax = plt.subplots(figsize=(10, 8), dpi=1000)
    ax.set_xlabel('Time [s]', fontsize=11, color='#154B5F')
    ax.tick_params(axis='x', colors='#154B5F')
    ax.set_ylabel('Pairs', fontsize=11, color='#154B5F')
    # Load pairwise dataset
    lns1, lns2 = None, None
    sum_collect, sum_wrong = 0, 0
    for idx_i, pairname in enumerate(pair_aircraft_dic.keys()):
        pairname2pred_answer[pairname] = list()
        pairname2pred[pairname] = list()
        pairname2target[pairname] = list()
        # Get each aircraft info
        aircraft_a, aircraft_b = pairname.split('&')
        if aircraft_a not in aircrafts_list:
            aircrafts_list.append(aircraft_a)
        if aircraft_b not in aircrafts_list:
            aircrafts_list.append(aircraft_b)
        ...
        for idx_j, p in enumerate(preds):
            ...
            had_conf = p[1].item()
            not_had_conf = p[0].item()
            pred_had_conf = True if had_conf > not_had_conf else False
            # Get answer
            conf_pair_list = conf_list_in_sec[sec]
            answer_had_conf = True if pairname in conf_pair_list else False
            # Plot (pairs -vs- time) & Count
            if pred_had_conf == answer_had_conf:
                pairname2pred_answer[pairname].append('Correct')
                if sum_collect == 0:
                    lns1 = ax.plot(idx_j, idx_i, marker='o', color='#00FF00', label='Correct')
                else:
                    ax.plot(idx_j, idx_i, marker='o', color='#00FF00')
                sum_collect += 1

            else:
                pairname2pred_answer[pairname].append('Wrong')
                if sum_wrong == 0:
                    lns2 = ax.plot(idx_j, idx_i, marker='o', color='#FF0000', label='Wrong')
                else:
                    ax.plot(idx_j, idx_i, marker='o', color='#FF0000')
                sum_wrong += 1
            # Append to list
            if answer_had_conf is True:
                pairname2target[pairname].append('Conf')
            else:
                pairname2target[pairname].append('Not Conf')
            
            if pred_had_conf is True:
                pairname2pred[pairname].append('Conf')
            else:
                pairname2pred[pairname].append('Not Conf')

    ax.set_title('Pred {} by NN: Avg Accuracy={:.2f}'.format(mode_name, sum_collect/ (sum_collect + sum_wrong)), loc='center', fontsize=15, color='#154B5F')
    # Put legend
    lns = lns1 + lns2
    labels = [l.get_label() for l in lns]
    legend = ax.legend(lns, labels, bbox_to_anchor=(0.01, 0.15), loc='upper left', borderaxespad=1, fontsize=15, facecolor="white")
    legend.set_zorder(13)
    ...
    plt.tight_layout()
    plt.savefig(pair_time_plot_fname)
    plt.close()
    # ----------------------------------------------------------------------------------------------------
    # 2nd Plot (Pairs -vs- Normalized Times) -------------------------------------------------------------
    print("Simulation Analysis using MLP, Bar Plot (Pairs - vs - Normalized Times), is started.")
    lns1, lns2, lns3 = None, None, None
    # Get max sec
    pairname2sec = dict()
    for k, v in pairname2pred_answer.items():
        pairname2sec[k] = len(v)

    max_sec = max(pairname2sec.values())
    # Padding
    for pairname in pairname2pred_answer.keys():
        if pairname in pairname2sec.keys():
            for i in range(pairname2sec[pairname], max_sec):
                pairname2pred_answer[pairname].append('Empty')
                pairname2target[pairname].append('Empty')
                pairname2pred[pairname].append('Empty')
        else:
    ...
    pair_time_vs_count_bar_fname = '{}[Normalized Time -vs- Pairs]'.format(file_name)
    fig, ax = plt.subplots(figsize=(10, 8), dpi=1000)
    ax.set_xlabel('Pairs', fontsize=11, color='#154B5F')
    ax.tick_params(axis='x', colors='#154B5F')
    ax.set_ylabel('Time [s]', fontsize=11, color='#154B5F')
    ax.set_ylim(0.0, 1.0)
    # Plot stack graph
    for idx_i, pairname in enumerate(pairname2pred_answer.keys()):
        # Count
        collect_num = pairname2pred_answer[pairname].count('Correct')
        wrong_num = pairname2pred_answer[pairname].count('Wrong')
        empty_num = pairname2pred_answer[pairname].count('Empty')
        # Normalize based on sec
        collect_num = collect_num / max_sec
        wrong_num = wrong_num / max_sec
        empty_num = empty_num / max_sec
        # Plot stack
        bottom = 0
        for idx_j, num in enumerate([collect_num, wrong_num, empty_num]):
            if idx_j == 0:
                bar_color = '#00FF00'
            elif idx_j == 1:
                bar_color = '#FF0000'
            ...
            if idx_i == 0:
                if idx_j == 0:
                    lns1 = ax.bar(idx_i, num, bottom=bottom, color=bar_color, label='Collect')
                if idx_j == 1:
                    lns2 = ax.bar(idx_i, num, bottom=bottom, color=bar_color, label='Wrong')
                if idx_j == 2:
                    lns3 = ax.bar(idx_i, num, bottom=bottom, color=bar_color, label='Empty')
            else:
                ax.bar(idx_i, num, bottom=bottom, color=bar_color)
            # Increment bottom
            bottom += num
    # Put legend
    lns = lns1 + lns2 + lns3
    labels = ['Collect', 'Wrong', 'Empty']
    #legend = ax.legend(lns, labels, loc='upper left', borderaxespad=1, fontsize=11, facecolor="white")
    ax.set_title('Pred {} by NN: Avg Accuracy={:.2f}'.format(mode_name, sum_collect/ (sum_collect + sum_wrong)), loc='center', fontsize=15, color='#154B5F')
    legend = ax.legend(lns, labels, bbox_to_anchor=(0.01, 0.15), loc='upper left', borderaxespad=1, fontsize=15, facecolor="white")
    legend.set_zorder(13)
    ...
    plt.tight_layout()
    plt.savefig(pair_time_vs_count_bar_fname)
    plt.close()
    # ----------------------------------------------------------------------------------------------------
    # 3rd Plot Table (UAV1 -vs- UAV2) -------------------------------------------------------------------
    print("Simulation Analysis using MLP, Table Plot (UAV1 - vs - UAV2), is started.")
    pair_time_table_dir = '{}[Time -vs- Pairs]'.format(file_name)
    if os.path.exists(pair_time_table_dir) is False:
        os.mkdir(pair_time_table_dir)
    # Init table
    sub_plot_size = math.ceil(math.sqrt(len(aircrafts_list)))
    axis = np.array(aircrafts_list, dtype=str)

    for idx, sec in enumerate(range(max_sec)):
        print('\t Creating table in {}sec / {}sec'.format(sec+1, max_sec))
        # Init
        sum_collect_in_sec = 0
        sum_wrong_in_sec = 0
        # Set figs
        ...
        table_fname = os.path.join(pair_time_table_dir, 'Positive-Negative result in {} sec'.format(sec+1))
        # Init table
        output_table = np.empty((len(aircrafts_list), len(aircrafts_list)), dtype=object)
        #output_color_table = np.empty((len(aircrafts_list), len(aircrafts_list)), dtype=str)
        output_color_table = np.full_like(output_table, "", dtype=object)

        for idx_i, aircraft_a in enumerate(aircrafts_list):
            for idx_j, aircraft_b in enumerate(aircrafts_list):
                pairname = '{}&{}'.format(aircraft_a, aircraft_b)
                if aircraft_a == aircraft_b:
                    output_color_table[idx_i, idx_j] = '#FFFFFF'
                # Get conf result
                if pairname in pairname2pred.keys():
                    pred = pairname2pred[pairname][sec]
                    target = pairname2target[pairname][sec]

                    if pred == 'Not Conf' and target == 'Conf': # False positive
                        #output_table[idx_i, idx_j] = 'False Positive'
                        output_color_table[idx_i, idx_j] = '#6495ED'
                        sum_wrong_in_sec += 1
                    elif pred == 'Conf' and target == 'Conf': # True positive
                        #output_table[idx_i, idx_j] = 'True Positive'
                        output_color_table[idx_i, idx_j] = '#00FF00'
                        sum_collect_in_sec += 1
                    elif pred == 'Conf' and target == 'Not Conf': # False negative
                        #output_table[idx_i, idx_j] = 'False Negative'
                        output_color_table[idx_i, idx_j] = '#FF00FF'
                        sum_wrong_in_sec += 1
                    elif pred == 'Not Conf' and target == 'Not Conf': # True negative
                        #output_table[idx_i, idx_j] = 'True Negative'
                        output_color_table[idx_i, idx_j] = '#FFFF00'
                        sum_collect_in_sec += 1
                    elif pred == 'Empty' and target == 'Empty': # Empty
                        output_color_table[idx_i, idx_j] = '#FFFFFF'
                else:
                    output_color_table[idx_i, idx_j] = '#FFFFFF'

        # Output table1
        ax1.set_title('Positive-Negative result in {} sec: Accuracy={:.2f}'.format(sec+1, sum_collect_in_sec/ (sum_collect_in_sec + sum_wrong_in_sec)),
                    loc='center', fontsize=15, color='#154B5F')
        ax1.axis('off')
        table1 = ax1.table(cellText=output_table, colLabels=axis, rowLabels=axis, cellColours=output_color_table,
                         loc='center')
        table1.auto_set_font_size(False)
        table1.set_fontsize(8)
        table1.scale(1, 1)
        # Output table2
        color_table_label = [['True Positive', 'True Negative', 'False Positive', 'False Negative']]
        color_table =[['#00FF00', '#FFFF00', '#6495ED', '#FF00FF']]
        ax2.axis('off')
        table2 = ax2.table(cellText=color_table_label, cellColours=color_table,
        ...
        plt.tight_layout()
        plt.savefig(table_fname, bbox_inches='tight',pad_inches = 0)
        plt.close()
...
# Exports NN phase between pair aircraft
def output_plot_with_pred_conf_from_NN(log_fname, conf_list_in_sec, mode='conf', use_entire_content=True):
    # About file path -------------------------------------------------------
    log_name = os.path.basename(log_fname)[:-4]
    # Load log file
    d = data_import(log_fname, 1)
    ...
    # Output csv
    plot_fname = os.path.join(output_csv_dir, 'USEPE_ML with predict NN {} {} for {}'.format(sep_category_name, count_category_name, log_name))
    # file_name, pair_aircraft_dic, pair_aircraft_sec_dic, mode, sep_category_name
    pairwise_aircraft_pred_NN_to_plot(file_name=plot_fname,
                                        pair_aircraft_dic=pair_aircraft_dic,
                                        pair_aircraft_sec_dic=pair_aircraft_sec_dic,
                                        conf_list_in_sec=conf_list_in_sec,
                                        mode=mode,
                                        sep_category_name=sep_category_name)